...
The rpistat service can load the configuration either from a local configuration file or from a single environment variable.
...
* **remoteConfigPath**          : Remote configuration path in which to search for the configuration data (e.g. "/config/rpistat"), passed to the custom remote loader
* **remoteConfigSecretKeyring** : Path to an optional secret keyring used by the custom remote loader to decrypt the remote configuration data (e.g. "/etc/rpistat/configkey.gpg")
* **remoteConfigData**          : Base64 encoded JSON configuration data to be used with the "envvar" provider (typically set via the RPISTAT_REMOTECONFIGDATA environment variable, which takes precedence over any value in the configuration file)
